    ...
    public function getEvents()
    {
        if ($this->session->access_token) {
            ...
            foreach ($events['items'] as $row) {
                ...
                $numberOfDays=$this->dateDiffInDays(substr($row['end']['dateTime'], 0, 10),substr($row['start']['dateTime'], 0, 10));
                $eventsArr[$arrCounter]['end'] = substr($row['end']['dateTime'], 0, 10). "T23:59:00";
//                if($numberOfDays >=1){
//                    $eventsArr[$arrCounter]['end'] = substr($row['end']['dateTime'], 0, 10). "T23:59:00";
//                }
//                else{
...
//                }
                $eventsArr[$arrCounter]['numberOfDays']=$numberOfDays;
                $arrCounter++;
            }
