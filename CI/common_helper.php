...
// GET ALL FIELDS DATA USING WHERE CONDITION

function dbQueryRowsFields($fields,$table, $whereArray = array(), $notWhereArray = array()) {
    ...
    $res = $CI->db->select($fields)->get($table)->result_array();
...
//get enquiry count
function getEnquiryCount($id)
{
     $CI =& get_instance();
     $CI->db->select('COUNT(car_id) as total');
     $CI->db->where('car_id',$id);
     $CI->db->group_by('car_id');
     $res=$CI->db->get('aa_messages')->row();
     return $res;
}
