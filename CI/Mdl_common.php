...
class Mdl_common extends CI_Model
{
	...
	function check_session(){	
		...
		if( $user== "" ){
			redirect('http://americasautosite.local:8080/americasautosite/');
		}
	}

    function check_artist_session()
    {
    ...
    function pagiationData($str,$num,$start,$segment,$per_page='20',$frame_surface_id=""){
		if($frame_surface_id!=0){
            $config['base_url'] = base_url().$str.'/s/'.$frame_surface_id;
        }else{
